...
docker volume prune
```
По умолчанию в качестве имени проекта используется имя директории, оно используется потом как имя образа и контейнера. Чтобы изменить имя проекта, можно отредактировать его в файле `config` перед созданием образа. Папка `www` с файлами сайта создаётся автоматически при создании контейнера, в той же папке что и `config`. Сейчас чтобы запустить другой сайт, нужно создать другой проект, и запускать контейнеры не одновременно, поскольку ip `127.0.0.1` у них тот же самый.
